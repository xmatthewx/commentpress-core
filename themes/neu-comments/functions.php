...
function commentpress_enqueue_scripts_and_styles() {
    ...
    wp_enqueue_style( 
        ...
        'neu_css', 
        get_template_directory_uri() . '/assets/css/neu'.$dev.'.css',
        array(),
        null, // version
        'all' // media
		...
		foreach( $comments_sorted AS $text_signature => $_comments ) {
			...
			switch( $text_signature ) {
				...
				case 'WHOLE_PAGE_OR_POST_COMMENTS':
					...
					$paragraph_text = __( 'the whole document', 'commentpress-core' );
					...
					switch( $current_type ) {
						...
						case 'page': $paragraph_text = __( 'the whole document', 'commentpress-core' ); break;
			...
			if ( $comment_count == 0 AND $text_signature == 'PINGS_AND_TRACKS' ) {
				...
				// show heading
				// @neu: edit comment heading
				echo '<h3 id="para_heading-'.$text_sig.'"'.$no_comments_class.'><a class="comment_block_permalink" title="'.$permalink_text.'" href="#para_heading-'.$text_sig.'">'.$heading_text.' <em class="neu-toggle">&rightarrow;</em></a></h3>'."\n\n";
				...
				if( in_array( $text_sig, $used_text_sigs ) ) {
					...
					if ( count( $_comments ) > 0 ) {
				        ...
				        /** 
				         * @neu: link to comment at top
				         * requires scrollto form
				         * @see cp_scroll_comments()
				         */
                        // if ( count( $_comments ) > 2 ) {
                        //                          echo '<a href="#reply_to_para-'.$para_num.'">Add a comment below.</a>';
                        //                         }
					...
					/**
                     * add to used array
                     * @neu: originally placed after if comments, before setup comment form
                     *
                     */
					...
					if ( 'open' == $post->comment_status AND $text_signature != 'PINGS_AND_TRACKS' ) {
						...
						if ( $allowed_to_comment ) {
							...
							$link_content = sprintf(
								__( 'Leave a comment on <em>%s</em>', 'commentpress-core' ),
								$paragraph_text
							);
							...
							// leave comment link
							// @neu: this is it...
							// @see: reply_to_para ... not logged-in, not unique  
							echo '<div class="reply_to_para" id="reply_to_para-'.$para_num.'">'."\n".
					...
					} // END setup comment form



					/**
                     * @neu: END making edits
                     *
                     */
